...
setup() {
        ...
        obs="$(docker container inspect -f '{{.State.Status}}' obs)"

        if [ "$obs" = running ]; then
                echo "Main OBS container is already running"
        else
                docker run --rm --detach --name obs -p 4444:1234 "$(docker build -q docker)"
        fi
        ...
        docker run --rm --detach --name obs-record -p 4445:1234 "$(docker build -q docker)"
        docker run --rm --detach --name obs-stream -p 4446:1234 "$(docker build -q docker)"
